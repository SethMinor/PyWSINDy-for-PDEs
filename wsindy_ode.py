...
class WSINDy:
  def __init__(self, U, alpha, beta, X, V=[], names=None, m=None, p=None, s=None, jacobian = 1.,
    ...
    if rescale:
      [self.yx, self.yt] = self.compute_temporal_scale()
  ...
  def optimal_support(self, d, changepoint, F_root):
    ...
    if self.verbosity:
      ...
      Uhat_d = Uhat_d.numpy()
      plt.plot(freqs, Uhat_d, 'r--', label=r'Spectrum, $\mathcal{F}_t[U]$')
  ...
  # Compute mask of query point indices, U[mask] = U[tk]
  def compute_query_points(self):
    subsamples = [subsample(self.s[i], self.m[i], self.X[i]) for i in range(self.U.ndim)]
    mask = tuple(map(torch.tensor, zip(*itertools.product(*subsamples))))
  ...
  # Compute temporal scale yt
  def compute_temporal_scale(self):
    max_t = max(1,max(tuple(ai[0] for ai in self.alpha)))

    # Ansatz given in the paper
    yt = (1 / (self.m[0] * self.spacing[0]) * (my_nchoosek(self.p[0], max_t/2)
  ...
  def compute_scale_matrix(self, powers, derivs):
    ...
    for j in range(num_terms):
      ...
      if (aj==None) or (bj==None):
        ...
        yt_term = yt**(self.alpha[0][0] - self.alpha[aj][0])
      
        yu_term = [yu[n]**(bj[n]) for n in range(len(yu))]
        yu_term = np.prod(yu_term)

        mu[j] = (yu_term * yt_term)/self.yu
    return mu
  ...
  def build_lhs(self, lhs_name):
    ...
    if self.rescale:
      lhs = compute_weak_poly(self.U, kernel, self.spacing, yu=self.yu, yxyt=self.yt, jacobian=self.jacobian)
    else:
  ...
  def print_report(self):
    ...
    print('RESULTS')
    ode = symbolic_ode(self.lhs_name, self.rhs_names, self.coeffs)
