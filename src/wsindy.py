...
class WSINDy:
  def __init__(self, U, alpha, beta, X, V=[], names=None, m=None, p=None, s=None, jacobian = 1.,
    ...
    self.init_kc_guess = init_kc_guess
  ...
  # Determines bandwidth of test functions by spectral matching
  ...
  def hyperparameter_sweep(self, lhs_name=None, m_values=None, Lambdas=None, rescales=(True, False),
                           library_fcn=None, truth=None, csv_path=None, plot=True):
    if lhs_name is None:
  ...
  def plot_sweep(self, results, metrics=None):
    ...
    for rescale in rescales:
      ...
      fig, ax = plt.subplots(1, len(metrics), figsize=(12,4), sharey=True)
      ax = np.atleast_1d(ax)
