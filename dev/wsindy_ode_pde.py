...
def symbolic_ode(lhs_name, rhs_names, w):
  return symbolic_pde(lhs_name, rhs_names, w)
...
class WSINDy:
  def __init__(self, U, alpha, beta, X, V=[], names=None, m=None, p=None, s=None, jacobian = 1.,
    ...
    if equation_type not in ('ode', 'pde'):
      raise ValueError("equation_type must be 'ode' or 'pde'.")
    ...
    self.spacing = [xi.diff()[0] for xi in self.X]
  ...
  def optimal_support(self, d, changepoint, F_root):
    ...
    if self.verbosity:
      ...
      Uhat_d = Uhat_d.numpy()
      label = r'Spectrum, $\mathcal{F}_t[U]$' if self.equation_type == 'ode' else r'Spectrum, $\mathcal{F}_d[U]$'
      plt.plot(freqs, Uhat_d, 'r--', label=label)
  ...
  def compute_query_points(self):
    ...
    if self.verbosity:
      if self.equation_type == 'ode':
        ...
        plt.figure(figsize=(6,3))
        plt.plot(t, self.U, 'r', label='$' + self.names[0] + '$')
        plt.scatter(tk, self.U[mask], color='black', marker='.', s=6, label='Query Points')
        plt.xlabel('$t$')
        plt.title('$' + self.names[0] + '(t)$')
        plt.legend(loc='upper right')
        plt.grid(True)
        plt.show()
      else:
  ...
  def get_derivative_names(self):
    ...
    for elem in self.alpha:
      if all(value == 0 for value in elem):
        ...
        if D == 0:
          derivative_names.append('_{'+'t'*elem[0]+'}')
        elif D == 1:
  ...
  def get_weight_fcns(self, d):
    ...
    if self.verbosity:
      ...
      if self.equation_type == 'ode':
        plt.xlabel('$t_k - t$')
        plt.ylabel(r'$\mathcal{D}^i\phi(t_k - t)$')
      else:
        plt.xlabel('$x_k - x$')
  ...
  def print_report(self):
    ...
    print('RESULTS')
    eq_label = self.equation_type.upper()
    eqn = symbolic_pde(self.lhs_name, self.rhs_names, self.coeffs)
  ...
  # Sweep hyperparameters (m, Lambda, rescale) and tabulate model results
  def hyperparameter_sweep(self, lhs_name=None, m_values=None, Lambdas=None,
                           rescales=(True, False), library_fcn=None, truth=None,
                           csv_path=None, plot=True):
    if lhs_name is None:
      lhs_name = getattr(self, 'lhs_name', self.names[0] + self.derivative_names[0])
    if m_values is None:
      m_values = []
      for factor in [0.5, 0.75, 1.0, 1.25, 1.5]:
        mf = [int(round(factor*mi)) for mi in self.m]
        mf = [min(max(mi,2), (self.U.shape[i]-1)//2) for i,mi in enumerate(mf)]
        if mf not in m_values:
          m_values.append(mf)
    else:
    ...
    for m in tqdm(m_values):
      for rescale in rescales:
        model = WSINDy(self.U, self.alpha, self.beta, self.X, V=self.V, names=self.names,
                       m=m, s=s, jacobian=self.jacobian, tau=self.tau, tau_hat=self.tau_hat,
                       init_guess=self.init_guess, verbosity=False, rescale=rescale,
        ...
        for Lambda in Lambdas:
          ...
          results['model'].append(symbolic_pde(lhs_name, rhs_names, w))
          ...
          # Max relative coefficient error, E_inf in the paper
          if truth is not None:
            got = dict(zip(results['terms'][-1], results['coeffs'][-1]))
            err = max(abs(got.get(name, 0.) - cj)/abs(cj) for name,cj in truth.items())
  ...
  # Heatmaps of sweep metrics over the (m, Lambda) plane
  def plot_sweep(self, results, metrics=None):
    ...
    titles = {'sparsity': 'Nonzero terms', 'R2': '$R^2$', 'coeff_err': r'$\log_{10}$ Coeff. Error'}
    ...
    for rescale in rescales:
      ...
      for j,metric in enumerate(metrics):
        ...
        if metric == 'R2':
          pcm = ax[j].pcolormesh(Lambdas, range(len(m_labels)), Z, shading='nearest', vmin=0, vmax=1)
        elif metric == 'coeff_err':
          Z = np.log10(np.maximum(Z, 1e-16))
          pcm = ax[j].pcolormesh(Lambdas, range(len(m_labels)), Z, shading='nearest')
        else:
          pcm = ax[j].pcolormesh(Lambdas, range(len(m_labels)), Z, shading='nearest')
        ax[j].set_xscale('log')
        ax[j].set_yticks(range(len(m_labels)))
        ax[j].set_yticklabels(m_labels)
